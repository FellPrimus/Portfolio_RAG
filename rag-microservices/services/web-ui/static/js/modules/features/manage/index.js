...
function renderBreadcrumb() {
    ...
    homeBtn.className = 'breadcrumb-home-btn';
    homeBtn.innerHTML = '🏠';
    homeBtn.title = '전체 문서로 이동';
    homeBtn.addEventListener('click', () => navigateToFolder(null));
    container.appendChild(homeBtn);

    // 상위 폴더 버튼 (루트가 아닌 경우에만)
    if (currentFolderId !== null) {
        const upBtn = document.createElement('button');
        upBtn.className = 'breadcrumb-up-btn';
        upBtn.innerHTML = '⬆️';
        upBtn.title = '상위 폴더로 이동';
...
async function handleDropToParent(e, parentFolderId) {
    ...
    try {
        if (parentFolderId) {
            // 상위 폴더로 이동: assignDocumentToFolder가 기존 폴더 제거를 원자적으로 처리
            await assignDocumentToFolder(docId, parentFolderId);
        } else {
            // 루트로 이동: 현재 폴더에서만 제거 (루트는 "폴더 없음" 상태)
            if (currentFolderId) {
...
async function moveDocumentToParent(docId) {
    ...
    try {
        const parentFolderId = folderPath.length > 1 ? folderPath[folderPath.length - 2].id : null;
        if (parentFolderId) {
            // 상위 폴더로 이동: 기존 폴더 제거가 백엔드에서 원자적으로 처리됨
            await assignDocumentToFolder(docId, parentFolderId);
        } else {
            // 루트로 이동
            await removeDocumentFromFolder(docId, currentFolderId);
