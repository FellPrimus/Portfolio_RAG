...
export async function askQuestionStream(question) {
    ...
    try {
        const secureMode = document.getElementById('secure-mode-checkbox').checked;
        const webSearchEnabled = document.getElementById('web-search-checkbox')?.checked || false;
...
function handleStreamEvent(eventType, data, context) {
    ...
    switch (eventType) {
        ...
        case 'web_verification':
            // 웹 검증 결과 저장
            let webMeta = context.getMetadata() || {};
            webMeta.web_verification_status = data.status;
            webMeta.web_confidence_delta = data.confidence_delta;
            webMeta.web_sources = data.web_sources || [];
            context.metadata(webMeta);

            // 웹 검증 진행 상태 표시
            if (statusTextDiv) {
                const statusMessages = {
                    'confirmed': '웹 검색 결과와 일치 확인됨',
                    'enhanced': '웹 검색으로 답변 보강 완료',
                    'conflicting': '웹 검색 결과와 일부 차이 있음',
                    'no_data': '관련 웹 검색 결과 없음'
                };
                statusTextDiv.textContent = statusMessages[data.status] || '웹 검증 완료';
            }
            break;
...
export function appendMetadata(answerContainer, metadata) {
    ...
    // 웹 검색 교차 검증 정보
    const webVerificationStatus = metadata.web_verification_status || 'skipped';
    const webConfidenceDelta = metadata.web_confidence_delta || 0;
    const webSources = metadata.web_sources || [];
    const webVerificationDetails = metadata.web_verification_details || null;
    const webVerificationHtml = buildWebVerificationHtml(webVerificationStatus, webConfidenceDelta, webSources, webVerificationDetails);
    ...
    const webSourcesHtml = buildWebSourcesHtml(webSources);

    const messageContent = messageWrapper.querySelector('.message-content');
    const timestamp = messageWrapper.querySelector('.message-timestamp');

    if (messageContent && timestamp) {
        messageContent.insertAdjacentHTML('afterend', qualityHtml + sourcesHtml + webSourcesHtml);
    } else {
        messageWrapper.innerHTML += qualityHtml + sourcesHtml + webSourcesHtml;
    }
